...
Synthea CLI (Java) → `data/synthea_raw/` (native Synthea CSV export)
        ↓ (`src/generator.py`: concept-dictionary mapping, NOTE templating, dirty-data injection)
...
Synthea's CSV export does not include free-text clinical notes. `note_text` is generated by `src/generator.py` from simple Python templates that reference each note's associated visit type, date, and (where applicable) condition. `person_id`, `note_date`, and `visit_occurrence_id` are still sourced from the mapped Synthea data.
