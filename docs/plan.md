...
1. Map selected Synthea CSV outputs into the simplified OMOP-style tables using `src/concepts.py` concept dictionaries, generate NOTE text from hardcoded phrases keyed to visit type, inject a small share of intentionally dirty rows, and write the result to `data/raw/`.
...
- generate NOTE records with `note_text` by randomly selecting from hardcoded complaint and assessment phrases keyed to visit type (outpatient, inpatient, ER)
